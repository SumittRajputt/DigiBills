...
from app.schemas.customer_plan import (
    ...
    CustomerPlanUpdateRequest,
)
from app.services.customer_plan_service import (
    CUSTOMER_CUSTOMER_TYPE,
    ensure_customer_plans,
    parse_json,
)


router = APIRouter(
    prefix="/admin/customer-plans",
    tags=["Admin Customer Plans"],
)
...
@router.get(
    "",
    response_model=list[CustomerPlanResponse],
)
...
def update_customer_plan(
    plan_id: str,
    request: CustomerPlanUpdateRequest,
    current_user: User = Depends(
        require_role("super_admin")
    ),
    db: Session = Depends(get_db),
    ...
    if plan is None:
        raise HTTPException(
            status_code=404,
            detail="Customer subscription plan not found.",
        )
    ...
    plan.per_bill_price = request.per_bill_price
